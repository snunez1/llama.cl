...
1. Get the models from Karpathy's repo ([Orig instructions](https://github.com/karpathy/llama2.c#feel-the-magic)) pretrained on [TinyStories](https://huggingface.co/datasets/roneneldan/TinyStories) the dataset.
...
4. Load system with `(ql:quickload :llama)` from the REPL
5. Move into the package `(in-package :llama)`
6. Initalise the models with `(init #P"stories15M.bin" #P"tokenizer.bin" 32000)` (adjust paths if neccessary)
7. Generate a story with: `(generate *model* *tokenizer*)`

You can experiment with temperature, prompts and various samplers.  See code for all the options.

## Performance

My machine is running a 2.6 GHz 6-core Intel Core i7 5930 six core processor, 256K/15MB cache with 64GB DDR4 RAM and with the `stories15M` I get about 2.5 tok/sec with CCL and 3.7 tok/s with SBCL.  Using LLA, the numbers are 14.4 tok/sec for CCL and 34.4 tok/sec for SBCL.

## Improvements

Please see the issues and discussion in this repo for possible ways to improve Common Lisp Llama inferencing.
...
Have you ever wanted to inference a baby [Llama 2](https://ai.meta.com/llama/) model in pure C? Well, now you can!
